...
const roleLinks: Record<string, [string, string][]> = {
  seller: [['/seller', 'Workspace'], ['/seller/products', 'Products'], ['/seller/orders', 'Orders'], ['/seller/wallet', 'Wallet'], ['/seller/disputes', 'Disputes']],
  rider: [['/rider', 'Wallet & home'], ['/rider#quotes', 'Quotes'], ['/rider#live-orders', 'Live orders'], ['/rider#completed', 'Completed'], ['/rider/wallet', 'Wallet']],
  admin: [['/admin', 'Overview'], ['/admin/sellers', 'Sellers'], ['/admin/riders', 'Riders'], ['/admin/store-areas', 'Store Areas'], ['/admin/delivery-areas', 'Delivery fees'], ['/admin/dispatch', 'Dispatch'], ['/admin/orders', 'Orders'], ['/admin/products', 'Products'], ['/admin/withdrawals', 'Payouts'], ['/admin/banners', 'Banners']],
}
...
export default async function SiteHeader() {
  ...
  try {
    ...
    // Render the public header if auth is unavailable.
  }
  ...
  const workspaceLinks = role ? roleLinks[role] || [] : []
    ...
    <header className="sticky top-0 z-40 border-b bg-white/95 backdrop-blur" style={{ borderColor: 'var(--line)' }}>
      <div className="bg-[#14251e] text-white">
        <div className="page-wrap flex h-9 items-center justify-between text-[11px] font-semibold tracking-wide">
          {workspace ? <p className="text-[#d9f33f]">{workspaceLabel} · secure operations</p> : <p className="ml-auto hidden text-white/70 sm:block">Secure shopping from trusted Nigerian sellers</p>}
        </div>
      </div>
      <div className="page-wrap flex min-h-[72px] items-center gap-2 py-3 sm:gap-4">
        ...
        <Link href={workspace ? workspaceHome : '/'} className="shrink-0" aria-label={workspace ? workspaceLabel : 'Jojokev home'}>
          ...
          <span className="hidden text-[9px] font-bold uppercase tracking-[.2em] text-slate-400 sm:block">{workspace ? (isSeller ? 'seller operations' : isRider ? 'rider operations' : 'admin operations') : 'shop smart. shop local.'}</span>
        </Link>
        ...
        {workspace && <nav className="ml-4 hidden flex-1 items-center gap-1 overflow-x-auto lg:flex">{workspaceLinks.map(([href, label]) => <Link key={href} href={href} className="whitespace-nowrap rounded-xl px-3 py-2 text-xs font-bold text-slate-600 transition hover:bg-[#e5f6ee] hover:text-[#0b5d43]">{label}</Link>)}</nav>}
      ...
      {workspace && <div className="page-wrap flex items-center gap-4 overflow-x-auto border-t py-2.5 text-xs font-bold text-slate-600 lg:hidden" style={{ borderColor: 'var(--line)' }}>{workspaceLinks.map(([href, label]) => <Link key={href} href={href} className="whitespace-nowrap hover:text-[#14805d]">{label}</Link>)}</div>}
    </header>
  )
}
