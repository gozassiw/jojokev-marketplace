...
export default function MobileMenu({ workspace, isSeller }: { workspace: boolean; isSeller: boolean; role: string | null }) {
  ...
  const workspaceLinks = isSeller
    ? [['Dashboard', '/seller'], ['Products', '/seller/products'], ['Orders', '/seller/orders'], ['Balance / payouts', '/seller/wallet']]
    : [['Overview', '/admin'], ['Sellers', '/admin/sellers'], ['Products', '/admin/products'], ['Orders', '/admin/orders'], ['Payouts', '/admin/withdrawals'], ['Banners', '/admin/banners']]
  ...
  const drawer = open && mounted ? createPortal(
    <div className="fixed inset-0 z-[9999] md:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
      ...
      <aside onClick={event => event.stopPropagation()} className="absolute bottom-0 left-0 top-0 flex w-[min(88vw,390px)] flex-col overflow-hidden bg-white shadow-2xl">
        <div className="flex h-[76px] shrink-0 items-center justify-between border-b px-5" style={{ borderColor: 'var(--line)' }}><Link href={workspace ? (isSeller ? '/seller' : '/admin') : '/'} onClick={close} className="text-[27px] font-black tracking-[-.08em] text-[#14805d]">jojokev<span className="text-[#ff7a28]">.</span></Link><button type="button" aria-label="Close menu" onClick={close} className="grid h-10 w-10 place-items-center rounded-full text-slate-700 hover:bg-slate-100"><CloseIcon /></button></div>

        {workspace ? <div className="min-h-0 flex-1 overflow-y-auto"><div className="border-b bg-[#14251e] px-5 py-5 text-white"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#d9f33f]">{isSeller ? 'Seller workspace' : 'Admin control room'}</p><p className="mt-2 text-lg font-black">{isSeller ? 'Run your shop' : 'Run the marketplace'}</p></div><div className="p-3">{workspaceLinks.map(([label, href]) => <Link key={href} href={href} onClick={close} className="flex items-center justify-between rounded-xl px-3 py-3.5 text-sm font-bold text-slate-700 hover:bg-[#e5f6ee] hover:text-[#0b5d43]"><span>{label}</span><ArrowIcon /></Link>)}<div className="my-3 border-t" style={{ borderColor: 'var(--line)' }} /><Link href="/" onClick={close} className="flex items-center justify-between rounded-xl px-3 py-3.5 text-sm font-bold text-[#14805d] hover:bg-[#e5f6ee]"><span>View storefront</span><ArrowIcon /></Link><Link href="/account" onClick={close} className="flex items-center justify-between rounded-xl px-3 py-3.5 text-sm font-bold text-slate-700 hover:bg-[#e5f6ee]"><span>My account</span><ArrowIcon /></Link></div></div> : <div className="min-h-0 flex-1 overflow-y-auto"><div className="border-b px-5 py-4"><Link href="/account" onClick={close} className="flex items-center justify-between py-2 text-base font-black text-slate-800"><span>My account</span><ArrowIcon /></Link><Link href="/orders" onClick={close} className="flex items-center justify-between py-2 text-sm font-semibold text-slate-700"><span>Orders</span><ArrowIcon /></Link><Link href="/account/addresses" onClick={close} className="flex items-center justify-between py-2 text-sm font-semibold text-slate-700"><span>Addresses</span><ArrowIcon /></Link></div><div className="px-5 py-5"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Categories</p><Link href="/search" onClick={close} className="text-xs font-bold text-[#d99032]">See all</Link></div>{buyerCategories.map(([icon, label, href]) => <Link key={label} href={href} onClick={close} className="flex items-center gap-4 py-3 text-sm font-semibold capitalize text-slate-700"><span className="grid h-8 w-8 place-items-center text-xl grayscale">{icon}</span><span>{label}</span></Link>)}</div><div className="border-t px-5 py-5"><div className="grid gap-2 text-sm font-semibold text-slate-700"><Link href="/sell/register" onClick={close} className="rounded-xl bg-[#14251e] px-4 py-3 text-center font-black text-[#d9f33f]">Start selling on Jojokev</Link><a href="mailto:support@jojokev.com" onClick={close} className="py-2">Contact us</a><Link href="/register" onClick={close} className="rounded-xl bg-[#14805d] px-4 py-3 text-center font-black text-white">Create a customer account</Link></div></div></div>}
      </aside>
    </div>,
    document.body,
  ) : null
